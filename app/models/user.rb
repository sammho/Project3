class User < ActiveRecord::Base
  ...
  def calculate_affinity(target_member)
    ...
    if affinity = UserAffinity.find_by_user_id_and_t_meetup_member_id(self.id, target_member.meetup_id)
      ...
      score = (affinity.twitter ? 10 : 0) +
                                (affinity.linked_in ? 20 : 0) +
                                50 * (@common_topics.count / (self.topics.count + 1)) +
                                20 * (normalized_common_topics_count / 20)
      ...
      puts "% common is #{50 * (@common_topics.count / (self.topics.count + 1))}\n"
      puts "Abs % is #{20 * (normalized_common_topics_count / 20)}\n"

      puts "Affinity of #{target_member.name} is #{score}\n"
